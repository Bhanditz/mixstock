...
\begin{figure}
<<echo=FALSE,fig=TRUE>>=
plot(mydata,stacklabels=TRUE,leg.space=0.4,leg.ncol=4)
@
...
\begin{figure}
<<echo=FALSE,fig=TRUE>>=
plot(lahanas98,stacklabels=TRUE)
@
...
if (!file.exists("umlboot.RData")) {
  mydata.umlboot <- genboot(mydata,"uml")
  save("mydata.umlboot",file="umlboot.RData",compress=TRUE)
...
of time ({\tt bugs.code="BB"} took \Sexpr{round(st2[3]/60,1)} minutes), 
